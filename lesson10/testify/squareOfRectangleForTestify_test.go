...
func TestSquareOfRectangle(t *testing.T) {
	...
	// Теперь жалею, что сделала домашнее задание после консультации, сейчас бы ещё спросила про моки, про которые наткнулась, пока читала про статьи про тестирование) Надеюсь, ещё в следующих курсах будет про это)
}
